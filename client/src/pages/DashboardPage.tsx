...
import {
  ...
  ChevronRightIcon, FlameIcon, ClockIcon, PlusIcon,
...
// ─── Stat Card ───────────────────────────────────────────────────────────────
function StatCard({ label, value, sub, to, Icon, sparkData }: {
  label: string; value: string; sub: string; to: string
  Icon: React.ComponentType<any>; sparkData?: number[]
}) {
  return (
    <Link to={to} style={{ textDecoration: 'none' }}>
      <div className="glass-card hoverable" style={{ padding: '18px', cursor: 'pointer', position: 'relative', overflow: 'hidden', height: '100%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
          <div style={{ width: '34px', height: '34px', borderRadius: '10px', background: 'var(--color-accent-dim)', border: '1px solid var(--color-accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-accent)' }}>
        ...
        {sparkData && sparkData.length > 1 && (
          <div style={{ marginTop: '4px' }}>
            <Sparkline data={sparkData} color="var(--color-accent)" height={28} />
          </div>
        )}
      </div>
    </Link>
...
function TxRow({ tx }: { tx: any }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
      <div style={{ width: '32px', height: '32px', borderRadius: '9px', background: isIncome ? 'rgba(16,185,129,0.12)' : 'rgba(244,63,94,0.10)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '0.85rem' }}>
        {isIncome ? '↑' : '↓'}
      </div>
      ...
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.85rem', fontWeight: 600, color: isIncome ? 'var(--color-success)' : 'var(--color-danger)', flexShrink: 0 }}>
...
function PokemonCard() {
  ...
  return (
    <div className="glass-card" style={{ padding: '20px', marginBottom: '24px', position: 'relative', overflow: 'hidden', border: `1px solid ${colors.hex}30` }}>
      ...
      <div style={{ position: 'absolute', bottom: '-20px', left: '-20px', width: '100px', height: '100px', borderRadius: '50%', background: colors.dim, filter: 'blur(30px)', pointerEvents: 'none' }} />

      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', position: 'relative' }}>
        {/* Sprite */}
        <div style={{ flexShrink: 0, width: '80px', height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {spriteUrl ? (
            <img src={spriteUrl} alt={pokemonName} className="poke-float"
              style={{ width: '80px', height: '80px', objectFit: 'contain', imageRendering: 'pixelated', filter: `drop-shadow(0 0 12px ${colors.hex}80)` }} />
          ) : (
            <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: colors.dim, border: `2px solid ${colors.hex}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem' }}>
              ⚡
            </div>
          )}
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
            ...
            <span className="poke-type-badge" style={{ '--color-accent': colors.hex, '--color-accent-dim': colors.dim } as any}>
              {type}
            </span>
          </div>
...
export default function DashboardPage() {
  ...
  const { data: txData } = useQuery<any>({
    ...
    queryFn: () => IS_PREVIEW ? Promise.resolve(MOCK_TRANSACTIONS) : api.get('/api/v1/ledger/transactions').then(r => r.data.data),
  })
  ...
  const addTaskMutation = useMutation({
    mutationFn: (title: string) => api.post('/api/v1/tasks', { title, date: today.toISOString().split('T')[0] }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tasks'] }); setQuickTask('') },
  })
  ...
  const weekDays = useMemo(() => Array.from({ length: 7 }, (_, i) => {
    ...
    const key = d.toISOString().split('T')[0]
    return { key, label: d.toLocaleDateString('en-US', { weekday: 'short' }).slice(0, 1), done: !!(history as any)[key] }
  }), [history])
  ...
  function handleQuickAdd(e: React.FormEvent) {
    ...
    if (IS_PREVIEW) { setQuickTask(''); return }
    addTaskMutation.mutate(quickTask.trim())
  }
  ...
  return (
    <PageLayout>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '24px' }}>
        <StatCard label="Net Balance" value={`৳${balance.toLocaleString()}`} sub="All time" to="/ledger" Icon={TrendingUpIcon} sparkData={balanceSparkData} />
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div className="glass-card" style={{ padding: '16px' }}>
            <div style={{ display: 'flex', gap: '10px', marginBottom: '14px' }}>
              <FinancePill label="Income" value={`৳${(summary.total_income / 1000).toFixed(0)}k`} color="#10B981" />
              <FinancePill label="Expenses" value={`৳${(summary.total_expenses / 1000).toFixed(0)}k`} color="#F43F5E" />
              <FinancePill label="Savings" value={`${savingsRate}%`} color="var(--color-accent)" />
            </div>
            {expenseSparkData.length > 1 && (
              <div>
                <div style={{ fontSize: '0.68rem', color: 'var(--color-text-muted)', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Monthly spend trend</div>
                <Sparkline data={expenseSparkData} color="#F43F5E" height={36} />
              </div>
            )}
          </div>
        </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 16px 12px' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ height: '4px', background: 'rgba(255,255,255,0.06)', borderRadius: '2px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${taskPct}%`, background: 'linear-gradient(90deg, var(--color-accent), var(--color-accent-glow))', borderRadius: '2px', transition: 'width 800ms ease-out' }} />
                </div>
              ...
              {streak > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', padding: '8px 12px', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <FlameIcon size={14} style={{ color: 'var(--color-warning)' }} />
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '1.1rem', fontWeight: 700, color: 'var(--color-warning)' }}>{streak}</span>
                  </div>
                  <span style={{ fontSize: '0.6rem', color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>streak</span>
                </div>
              )}
            </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px' }}>
                        ...
                        {n.tag_label && <span style={{ fontSize: '0.6rem', color: 'var(--color-text-muted)', background: 'rgba(255,255,255,0.06)', padding: '1px 5px', borderRadius: '4px' }}>{n.tag_label}</span>}
                      </div>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div className="glass-card" style={{ padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            {weekDays.map(({ key, label, done }) => (
              <div key={key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '10px', background: done ? 'var(--color-accent-dim)' : 'rgba(255,255,255,0.04)', border: `1px solid ${done ? 'var(--color-accent)' : 'rgba(255,255,255,0.08)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: done ? '0 0 10px var(--color-accent-glow)' : 'none', transition: 'all 200ms' }}>
                  {done ? <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="var(--color-accent)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 6l3 3 5-5" /></svg>
          ...
          {streak > 0 && (
            <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <FlameIcon size={13} style={{ color: 'var(--color-warning)' }} />
              <span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>
                <span style={{ color: 'var(--color-warning)', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{streak}</span>-day streak — keep it going
              </span>
            </div>
          )}
        </div>
      ...
      <div style={{ marginBottom: '8px' }}>
        ...
        <form onSubmit={handleQuickAdd} style={{ display: 'flex', gap: '8px' }}>
          <input value={quickTask} onChange={e => setQuickTask(e.target.value)} placeholder="Add a task for today…"
            className="input-accent"
            style={{ flex: 1, padding: '11px 14px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: 'var(--color-text-primary)', fontFamily: 'var(--font-ui)', fontSize: '0.875rem', outline: 'none' }} />
          <button type="submit" disabled={!quickTask.trim()} className="btn-primary"
            style={{ padding: '11px 16px', borderRadius: '12px', fontSize: '0.875rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
